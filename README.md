...
- 更新失敗・検証失敗・デプロイ確認失敗・定時監視による再実行起動は、Chatworkマイチャット `398449612` のみに通知します。通知は「何が起きたか・影響・推定原因・確認方法」を日本語で表示し、調査用の原文は末尾の技術詳細に残します。メール通知は使用しません
...
- 定時更新が作成されなかった場合は、監視ワークフローが `workflow_dispatch` で日次更新を直接起動します。API失敗時は最大3回再試行し、実行履歴の作成を確認できた場合だけ再実行開始を通知します
