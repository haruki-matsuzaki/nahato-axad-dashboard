...
- 案件別データと管理シートは行数を固定せず `A:ZZ` で取得し、3,000行（管理シートは2,000行）または最終列 `ZZ` に到達した場合は、範囲外の取得漏れを避けるため更新を停止します
- 更新前にスプレッドシートID、タイトル内の対象月、必須タブ名を照合します。同月に同優先度の異なるシートが複数見つかった場合は自動選択せず更新を停止します
...
- Codex automationの実行が遅延して12:30〜20:30 JSTを外れた場合、ローカル外部監視はデータ取得・Chatwork通知を行わず正常終了します
- `30 4,7,10 * * *`: 13:30 / 16:30 / 19:30 JST。Cloudflare Secret設定後に使う予備監視。Worker側でもこの3時刻以外の定期実行をデータ取得前に停止する
...
- 月初の対象シートが未登録の場合、土日祝を考慮した最初の営業日15:30 JSTまでは「準備待ち」として日次更新・再実行・Chatwork通知を停止します。15:30以降も未登録の場合は次の監視で異常として扱います
...
- `◆案件/媒体別日次_全体` と `◆案件別日次_全体_固定用` を更新後に再取得し、生成した月別JSONと案件・媒体・日付・売上・粗利・消化金額・件数を直接照合します
- 媒体明細だけに存在する途中追加案件は、媒体明細から案件合計を再計算して照合します。欠落・余分な行・数値差分・構造不備が1件でもあれば更新失敗として既存の5分間隔リトライへ送ります
- 直接照合結果と原本/生成JSONのSHA-256フィンガープリントは `data/source-audit-status.json` に保存します
...
- 更新失敗・検証失敗・デプロイ確認失敗・定時監視による再実行起動失敗は、Chatworkマイチャット `398449612` のみに通知します。自動再実行を正常に開始できた場合は通知しません。通知は「何が起きたか・影響・推定原因・確認方法」を日本語で表示し、調査用の原文は末尾の技術詳細に残します。メール通知は使用しません
...
- 定時更新が作成されなかった場合は、監視ワークフローが `workflow_dispatch` で日次更新を直接起動します。API失敗時は最大3回再試行し、再実行を開始できなかった場合だけ通知します
